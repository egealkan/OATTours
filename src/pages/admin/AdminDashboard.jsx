...
const AdminDashboard = () => {
    ...
    // Post-trip toggle state
    const [showPostTrip, setShowPostTrip] = useState(true);
    const [isTogglingPostTrip, setIsTogglingPostTrip] = useState(false);
    ...
    const fetchSettings = async () => {
        ...
        try {
            ...
            if (data) {
                ...
                // Default to true if the column value is null
    ...
    const handleTogglePostTrip = async (e) => {
        const newValue = e.target.checked;
        setShowPostTrip(newValue);
        setIsTogglingPostTrip(true);

        try {
            const { error } = await supabase.from('settings').upsert({
                id: 1,
                show_post_trip: newValue
            }, { onConflict: 'id' });

            if (error) throw error;

            toast.success(
                newValue ? 'Post Trip page is now visible to guests.' : 'Post Trip page is now hidden from guests.'
            );
        } catch (error) {
            console.error(error);
            toast.error('Failed to update setting.');
            setShowPostTrip(!newValue); // revert on error
        } finally {
            setIsTogglingPostTrip(false);
        }
    };
                ...
                <div className="dashboard-card security-card">
                    ...
                    <div className="feature-toggle-row">
                        <div className="toggle-info">
                            <strong>Post Trip Extension</strong>
                            <span>
                                {showPostTrip
                                    ? 'Guests can see the Post Trip page'
                                    : 'Post Trip page is hidden from guests'}
                            </span>
                        </div>
                        <label className="toggle-switch" title="Toggle Post Trip visibility">
                            <input
                                type="checkbox"
                                checked={showPostTrip}
                                onChange={handleTogglePostTrip}
                                disabled={isTogglingPostTrip}
                            />
