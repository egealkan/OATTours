...
const AdminWelcomeMeetingEdit = () => {
    ...
    const [tourData, setTourData] = useState({
        ...
        selected_hotel_ids: [] // NEW STATE FOR HOTEL CHECKLIST
    });
        ...
        try {
            ...
            if (activeTour) {
                ...
                setTourData({
                    ...
                    selected_hotel_ids: activeTour.selected_hotel_ids || [] // LOAD SELECTED HOTELS
                });
            }
    ...
    const handleHotelToggle = (hotelId) => {
        setTourData(prev => {
            const currentSelected = prev.selected_hotel_ids || [];
            // Using String comparison to avoid any integer/UUID strict equality issues
            const strId = String(hotelId);
            if (currentSelected.some(id => String(id) === strId)) {
                return { ...prev, selected_hotel_ids: currentSelected.filter(id => String(id) !== strId) };
            } else {
                return { ...prev, selected_hotel_ids: [...currentSelected, hotelId] };
            }
        });
    };
        ...
        try {
            if (activeTourId) {
                const cleanTourData = {
                    ...
                    selected_hotel_ids: tourData.selected_hotel_ids || [] // SAVE SELECTED HOTELS
                };
    ...
    return (
        <div className="admin-edit-wrapper">
            ...
            <form className="edit-form-card" onSubmit={handleSaveAll}>
                ...
                {/* 6. CURRENT TOUR HOTELS CHECKLIST (NEW SECTION) */}
                <div className="form-section">
                    <h3>Select Tour Hotels</h3>
                    <p className="hint-text">Check the hotels that are part of this specific tour. Only these will be shown to the guests.</p>
                    <div className="hotel-checklist">
                        {hotelsRoster.length > 0 ? (
                            hotelsRoster.map(hotel => (
                                <label key={hotel.id} className="checkbox-item">
                                    <input
                                        type="checkbox"
                                        checked={(tourData.selected_hotel_ids || []).some(id => String(id) === String(hotel.id))}
                            ...
                            <p className="empty-text">No hotels in dictionary yet. Add them below.</p>
                        )}
                    </div>
                </div>

                {/* 7. HOTEL DICTIONARY CRUD */}
                <div className="form-section">
