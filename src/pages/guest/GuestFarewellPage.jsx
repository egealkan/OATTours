  ...
  const fetchFarewellData = async () => {
    try {
      ...
      const { data: days } = await supabase
        .from('tour_days')
        .select('id, is_post_trip')
        .eq('tour_id', activeTour.id);
      
      // THE FIX: This is exactly what worked on the Welcome page.
      // We check for both boolean 'true' and string '"true"'.
      const dayIds = (days || [])
        .filter(d => d.is_post_trip !== true && d.is_post_trip !== 'true')
        .map(d => d.id);
...
//       // 4. Auto-Generate Itinerary Summary (with Images)
//       // Tell Supabase to ONLY fetch days where is_post_trip is explicitly false
//       const { data: days } = await supabase
//         .from('tour_days')
//         .select('id')
//         .eq('tour_id', activeTour.id)
