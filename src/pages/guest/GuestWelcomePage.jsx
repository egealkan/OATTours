...
const GuestWelcomePage = () => {
  ...
  useEffect(() => {
    const fetchWelcomeInfo = async () => {
      ...
      try {
        const { data: tourData, error: tourError } = await supabase
          .from('tours').select('climate_info').eq('is_active', true).single();
        if (tourError && tourError.code !== 'PGRST116') console.error(tourError);
        else if (tourData) setClimateInfo(tourData.climate_info);
        ...
        const { data: guideData } = await supabase
          .from('guide_profile').select('*').limit(1).single();
        if (guideData) setGuideProfile(guideData);

        const { data: settingsData } = await supabase
          .from('settings').select('pre_trip_info_url, pre_trip_info_description').eq('id', 1).single();
        if (settingsData) {
          setPreTripLink({
            url: settingsData.pre_trip_info_url || '',
            description: settingsData.pre_trip_info_description || ''
  ...
  useEffect(() => {
    const anyOpen = isClimateModalOpen || isGuideBioModalOpen || isPhotoModalOpen;
    document.body.style.overflow = anyOpen ? 'hidden' : 'unset';
  }, [isClimateModalOpen, isGuideBioModalOpen, isPhotoModalOpen]);

  if (isLoading) return <div className="loading-spinner">Loading Pre-Trip Information...</div>;
    ...
    <div className="guest-page-wrapper">
        ...
        <div className="info-card guide-card">
          ...
          <div className="guide-content-flex">
            ...
            <div className="guide-details">
              ...
              {bioIsLong && (
                <button className="btn-open-modal" style={{ marginTop: '10px' }} onClick={() => setIsGuideBioModalOpen(true)}>
                  Read Full Bio
                </button>
              )}
            </div>
      ...
      {preTripLink.url && (
        <div className="info-card center-card">
          <h2>📋 Pre-Trip Essentials</h2>
          <p className="climate-summary">
            {preTripLink.description || 'Find detailed information about airport meeting points, what to pack, useful tips, and more.'}
          </p>
          <a href={preTripLink.url} target="_blank" rel="noopener noreferrer"
            className="btn-open-modal" style={{ display: 'inline-block', textDecoration: 'none' }}>
            View Pre-Trip Guide →
          </a>
        </div>
      )}
      ...
      <div className="info-card center-card">
        ...
        <button className="btn-open-modal" onClick={() => setIsClimateModalOpen(true)}>
          Read Climate Information
        </button>
      </div>
          ...
          {contacts.transfers.length > 0 ? (
            <div className="contacts-grid">
              {contacts.transfers.map(contact => (
                <div key={contact.id} className="contact-item">
                  <span className="contact-name">{contact.name}</span>
                  <a href={`tel:${contact.phone_number}`} className="contact-phone">{contact.phone_number}</a>
                </div>
              ))}
            </div>
          ) : <p className="empty-text">Transfer personnel not assigned yet.</p>}
      ...
      {/* ── PROFILE PHOTO MODAL ── */}
      {isPhotoModalOpen && guideProfile?.profile_image_url && (
        <div className="modal-overlay" onClick={() => setIsPhotoModalOpen(false)}>
          <div className="photo-modal-container" onClick={e => e.stopPropagation()}>
            ...
            <img src={guideProfile.profile_image_url} alt={guideProfile.name} className="photo-modal-img" />
            {guideProfile.name && <p className="photo-modal-caption">{guideProfile.name}</p>}
          </div>
        </div>
      )}
